package cn.weegoo.core.servlet;
...
public class UserfilesDownloadServlet extends HttpServlet {
    ...
    public void fileOutputStream(HttpServletRequest req, HttpServletResponse resp) {
        ...
        int index = filepath.indexOf ( WeeGooProperties.USERFILES_BASE_URL );
        if ( index >= 0 ) {
            filepath = filepath.substring ( index + WeeGooProperties.USERFILES_BASE_URL.length ( ) );
        }

        filepath = UriUtils.decode ( filepath, "UTF-8" );
        WeeGooProperties weegooProperites = SpringUtil.getBean ( WeeGooProperties.class );
        File file = new File ( weegooProperites.getUserfilesBaseDir ( ) + WeeGooProperties.USERFILES_BASE_URL + filepath );
